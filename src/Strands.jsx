...
void main() {
  ...
  // One continuous center-weighted field. The cosine envelope repeats on wide
  // canvases and visually splits the strands into disconnected side clusters.
  float horizontalFade = 1.0 - smoothstep(0.18, 1.45, abs(uv.x));
  float env = pow(max(horizontalFade, 0.0), uTaper);
  vec3 col = vec3(0.0);
...
export default function Strands({
  ...
  propsRef.current = {
    ...
    dispersion, glassSize, reducedMotion,
  };
  ...
  useEffect(() => {
    ...
    const update = (time) => {
      ...
      program.uniforms.uTime.value = current.reducedMotion ? 0 : time * 0.001;
      ...
      if (!current.reducedMotion) animationId = requestAnimationFrame(update);
    };
